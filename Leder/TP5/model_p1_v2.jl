...
# variables
@variable(model, x[p in P, e_bar in E_bar, t in T], Bin)
...
@constraint(model, [p in P], sum(x[p,(l_max,1,(0,k)),1] for k in C) == z[p])      # state 1 based on the state 0
@constraint(model, [p in P, j_bar in V_bar, t in collect(1:t_max-1)], sum(x[p,(i_bar,j_bar),t] for i_bar in V_bar if (i_bar,j_bar) in E_bar) == sum(x[p,(j_bar,i_bar),t+1] for i_bar in V_bar if (j_bar,i_bar) in E_bar))     # flow-conservation
@constraint(model, [j in C, t in T], sum(x[p,(l,a,(i,j)),t]*R[(l,a,(i,j))] for p in P for l in L for a in A for i in V if (l,a,(i,j)) in keys(R)) >= D[(j,t)]/20) #demande
