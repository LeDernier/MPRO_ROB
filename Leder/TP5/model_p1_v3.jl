...
@constraint(model, [p in P, t in collect(1:t_max)], sum(x[p,l,a,e,t] for l in L for a in A for e in E if !((l,a,e) in V_bar)) == 0)  # unnecesary (l,a,(i,j)) by node relationship
...
@constraint(model, [p in P, t in collect(1:t_max-1)], x[p,l_max,0,(0,0),t] <= sum(x[p,l_max,1,(0,k),t+1] for k in C))
@constraint(model, [p in P, t in collect(1:t_max-1), j in C], sum(x[p,l,a_max,(i,j),t] for l in L for i in V if (i,j) in E) <= x[p,1,0,(j,0),t+1])

# flow-conservation constraints
@constraint(model, [p in P, t in collect(1:t_max-1), j in V], sum(x[p,l,a,(i,j),t] for l in L for a in A for i in V if (i,j) in E) == sum(x[p,l,a,(j,i),t+1] for l in L for a in A for i in V if (j,i) in E))
